...
/****** Object:  Job [E-mail: Alerta Capta Processos]    Script Date: 12/14/2012 11:24:47 ******/
BEGIN TRANSACTION
DECLARE @ReturnCode INT
SELECT @ReturnCode = 0
/****** Object:  JobCategory [[Uncategorized (Local)]]]    Script Date: 12/14/2012 11:24:47 ******/
...
EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N'E-mail: Alerta Capta Processos', 
		@enabled=0, 
		...
		@notify_level_page=0, 
		@delete_level=0, 
		@description=N'Alerta do Capta Processos parados nos clientes.', 
		@category_name=N'[Uncategorized (Local)]', 
		@owner_login_name=N'capta', @job_id = @jobId OUTPUT
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [STEP_CURSOR_ENVIAR_ALERTA_CAPTA_PROCESSOS]    Script Date: 12/14/2012 11:24:48 ******/
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'STEP_CURSOR_ENVIAR_ALERTA_CAPTA_PROCESSOS', 
		@step_id=1, 
		...
		@command=N'DECLARE CUR_MON_CAPTA_PROCESSOS CURSOR
FOR
SELECT CLIENTE, SQLINSTANCE, SQLHOSTNAME, IPSQL, NICKNAME FROM tb_monitor_cptagend WHERE NR_PROCESSOS = 0 AND EXPRESS = 1 --AND CLIENTE != ''SOLELUNA''
...
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
EXEC @ReturnCode = msdb.dbo.sp_add_jobschedule @job_id=@jobId, @name=N'SCHEDULE_ALERTA_CAPTA_PROCESSOS', 
		@enabled=1, 
		@freq_type=4, 
		@freq_interval=1, 
		@freq_subday_type=4, 
		...
		@active_start_time=60230, 
		@active_end_time=30000, 
		@schedule_uid=N'657d10ee-277b-4540-9a96-e60c34dabe3c'
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
...
/****** Object:  Job [E-mail: Alerta Capta Processos - DBA]    Script Date: 12/14/2012 11:24:48 ******/
BEGIN TRANSACTION
DECLARE @ReturnCode INT
SELECT @ReturnCode = 0
/****** Object:  JobCategory [[Uncategorized (Local)]]]    Script Date: 12/14/2012 11:24:48 ******/
IF NOT EXISTS (SELECT name FROM msdb.dbo.syscategories WHERE name=N'[Uncategorized (Local)]' AND category_class=1)
BEGIN
EXEC @ReturnCode = msdb.dbo.sp_add_category @class=N'JOB', @type=N'LOCAL', @name=N'[Uncategorized (Local)]'
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
...
DECLARE @jobId BINARY(16)
EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N'E-mail: Alerta Capta Processos - DBA', 
		@enabled=1, 
		@notify_level_eventlog=0, 
		@notify_level_email=0, 
		@notify_level_netsend=0, 
		...
		@owner_login_name=N'capta', @job_id = @jobId OUTPUT
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [step]    Script Date: 12/14/2012 11:24:48 ******/
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'step', 
		...
		@retry_interval=0, 
		@os_run_priority=0, @subsystem=N'TSQL', 
		@command=N'if exists (select top 1 cliente from tb_monitor_cptagend where dt_alt <= dateadd(hour, -1, getdate()) ) begin

declare @TableHTML varchar(max)

SET @TableHTML =    
	''
		<font face="Verdana" size="4">Monitoria do Capta Processos parado a mais de uma hora:</font>  
		...
		<tr>  
			<td width="27%" height="22" bgcolor="#000080">
				<b>  
			...
			<td width="39%" height="22" bgcolor="#000080">
				<b>  
					<font face="Verdana" size="2" color="#FFFFFF">
						SQLINSTANCE
					</font>
			...
			<td width="39%" height="90" bgcolor="#000080">
				<b>  
					<font face="Verdana" size="2" color="#FFFFFF">
						IPSQL
					</font>
	...
	''<tr>
		<td>
			<font face="Verdana" size="1">'' + 
				ISNULL(CONVERT(VARCHAR(100), cliente), '''') + ''
			</font>
		</td>'' +    
		''<td>
			<font face="Verdana" size="1">'' + 
				ISNULL(CONVERT(VARCHAR(100), sqlinstance),'''') + ''
			</font>
		</td>'' +
		''<td>
			<font face="Verdana" size="1">'' + 
				ISNULL(CONVERT(VARCHAR(100), sqlhostname),'''') + ''
			</font>
		</td>'' +
		''<td>
			<font face="Verdana" size="1">'' + 
				ISNULL(CONVERT(VARCHAR(100), ipsql),'''') + ''
			</font>
		</td>'' +
		''<td>
			<font face="Verdana" size="1">'' + 
				ISNULL(CONVERT(VARCHAR(100), dt_alt),'''') + ''
			</font>
		</td>'' +
		''<td>
			<font face="Verdana" size="1">'' + 
				ISNULL(CONVERT(VARCHAR(100), express),'''') + ''
			</font>
		</td>''
from tb_monitor_cptagend where dt_alt <= dateadd(hour, -1, getdate())
...
EXECUTE MSDB..SP_SEND_DBMAIL
	...
	@subject = ''Monitoria do Capta Processos Parado'',
	@body = @TableHTML,
	@body_format = ''HTML''
end', 
		@database_name=N'MonitorDBA', 
...
EXEC @ReturnCode = msdb.dbo.sp_add_jobschedule @job_id=@jobId, @name=N'Agenda', 
		...
		@freq_relative_interval=0, 
		@freq_recurrence_factor=0, 
		@active_start_date=20121122, 
		@active_end_date=99991231, 
		@active_start_time=80000, 
		@active_end_time=180000, 
		@schedule_uid=N'a97f0800-ad23-44e2-bb74-05faa2c90828'
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
EXEC @ReturnCode = msdb.dbo.sp_add_jobserver @job_id = @jobId, @server_name = N'(local)'
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
COMMIT TRANSACTION
...
/****** Object:  Job [E-mail: Alerta se tem Banco Suspect/Offline]    Script Date: 12/14/2012 11:24:48 ******/
BEGIN TRANSACTION
DECLARE @ReturnCode INT
SELECT @ReturnCode = 0
/****** Object:  JobCategory [[Uncategorized (Local)]]]    Script Date: 12/14/2012 11:24:48 ******/
...
EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N'E-mail: Alerta se tem Banco Suspect/Offline', 
		...
		@owner_login_name=N'capta', @job_id = @jobId OUTPUT
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [passo1]    Script Date: 12/14/2012 11:24:48 ******/
...
/****** Object:  Job [E-mail: Aviso Backup]    Script Date: 12/14/2012 11:24:48 ******/
BEGIN TRANSACTION
DECLARE @ReturnCode INT
SELECT @ReturnCode = 0
/****** Object:  JobCategory [[Uncategorized (Local)]]]    Script Date: 12/14/2012 11:24:48 ******/
...
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [passo1]    Script Date: 12/14/2012 11:24:49 ******/
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'passo1', 
		@step_id=1, 
		@cmdexec_success_code=0, 
		@on_success_action=1, 
...
/****** Object:  Job [E-mail: Aviso Defrag]    Script Date: 12/14/2012 11:24:49 ******/
BEGIN TRANSACTION
DECLARE @ReturnCode INT
SELECT @ReturnCode = 0
/****** Object:  JobCategory [[Uncategorized (Local)]]]    Script Date: 12/14/2012 11:24:49 ******/
IF NOT EXISTS (SELECT name FROM msdb.dbo.syscategories WHERE name=N'[Uncategorized (Local)]' AND category_class=1)
BEGIN
EXEC @ReturnCode = msdb.dbo.sp_add_category @class=N'JOB', @type=N'LOCAL', @name=N'[Uncategorized (Local)]'
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
...
/****** Object:  Job [Email: Aviso Integridade dos Backups]    Script Date: 12/14/2012 11:24:49 ******/
BEGIN TRANSACTION
DECLARE @ReturnCode INT
SELECT @ReturnCode = 0
/****** Object:  JobCategory [[Uncategorized (Local)]]]    Script Date: 12/14/2012 11:24:49 ******/
...
EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N'Email: Aviso Integridade dos Backups', 
		...
		@owner_login_name=N'capta', @job_id = @jobId OUTPUT
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [.]    Script Date: 12/14/2012 11:24:49 ******/
...
/****** Object:  Job [E-mail: Banco Suspect/Offline]    Script Date: 12/14/2012 11:24:49 ******/
BEGIN TRANSACTION
DECLARE @ReturnCode INT
SELECT @ReturnCode = 0
/****** Object:  JobCategory [[Uncategorized (Local)]]]    Script Date: 12/14/2012 11:24:49 ******/
...
/****** Object:  Job [E-mail: BD com índice acima 40%]    Script Date: 12/14/2012 11:24:49 ******/
BEGIN TRANSACTION
DECLARE @ReturnCode INT
SELECT @ReturnCode = 0
/****** Object:  JobCategory [[Uncategorized (Local)]]]    Script Date: 12/14/2012 11:24:49 ******/
...
/****** Object:  Job [E-mail: cversaos e cversis]    Script Date: 12/14/2012 11:24:49 ******/
BEGIN TRANSACTION
DECLARE @ReturnCode INT
SELECT @ReturnCode = 0
/****** Object:  JobCategory [[Uncategorized (Local)]]]    Script Date: 12/14/2012 11:24:49 ******/
...
/****** Object:  Job [E-mail: Espaço Livre menor que espaço do tamanho dos bancos]    Script Date: 12/14/2012 11:24:49 ******/
BEGIN TRANSACTION
DECLARE @ReturnCode INT
SELECT @ReturnCode = 0
/****** Object:  JobCategory [[Uncategorized (Local)]]]    Script Date: 12/14/2012 11:24:49 ******/
...
/****** Object:  Job [Email: Jobs Ativos]    Script Date: 12/14/2012 11:24:49 ******/
BEGIN TRANSACTION
DECLARE @ReturnCode INT
SELECT @ReturnCode = 0
/****** Object:  JobCategory [[Uncategorized (Local)]]]    Script Date: 12/14/2012 11:24:49 ******/
...
DECLARE @jobId BINARY(16)
EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N'Email: Jobs Ativos', 
		@enabled=1, 
		@notify_level_eventlog=0, 
		@notify_level_email=0, 
		@notify_level_netsend=0, 
		...
		@owner_login_name=N'capta', @job_id = @jobId OUTPUT
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [.]    Script Date: 12/14/2012 11:24:50 ******/
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'.', 
		@step_id=1, 
		@cmdexec_success_code=0, 
		@on_success_action=1, 
...
EXEC @ReturnCode = msdb.dbo.sp_add_jobschedule @job_id=@jobId, @name=N'.', 
		...
		@active_start_time=70000, 
		@active_end_time=235959, 
		@schedule_uid=N'db45bbef-8e2c-458a-acc7-33b4e3196fb9'
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
EXEC @ReturnCode = msdb.dbo.sp_add_jobserver @job_id = @jobId, @server_name = N'(local)'
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
COMMIT TRANSACTION
...
/****** Object:  Job [E-mail: Log de Banco de dados Grande]    Script Date: 12/14/2012 11:24:50 ******/
BEGIN TRANSACTION
DECLARE @ReturnCode INT
SELECT @ReturnCode = 0
/****** Object:  JobCategory [[Uncategorized (Local)]]]    Script Date: 12/14/2012 11:24:50 ******/
IF NOT EXISTS (SELECT name FROM msdb.dbo.syscategories WHERE name=N'[Uncategorized (Local)]' AND category_class=1)
BEGIN
EXEC @ReturnCode = msdb.dbo.sp_add_category @class=N'JOB', @type=N'LOCAL', @name=N'[Uncategorized (Local)]'
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
...
EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N'E-mail: Log de Banco de dados Grande', 
		...
		@owner_login_name=N'capta', @job_id = @jobId OUTPUT
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [passo1]    Script Date: 12/14/2012 11:24:50 ******/
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'passo1', 
		@step_id=1, 
		@cmdexec_success_code=0, 
		@on_success_action=1, 
...
/****** Object:  Job [E-mail: Memoria - CPU Clientes]    Script Date: 12/14/2012 11:24:50 ******/
BEGIN TRANSACTION
DECLARE @ReturnCode INT
SELECT @ReturnCode = 0
/****** Object:  JobCategory [[Uncategorized (Local)]]]    Script Date: 12/14/2012 11:24:50 ******/
...
DECLARE @jobId BINARY(16)
EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N'E-mail: Memoria - CPU Clientes', 
		@enabled=1, 
		@notify_level_eventlog=0, 
		@notify_level_email=0, 
		@notify_level_netsend=0, 
...
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'.', 
		...
		@retry_interval=0, 
		@os_run_priority=0, @subsystem=N'TSQL', 
		@command=N'select mc.codcli, c.snome as nome_cli, mc.servidor, mc.cpu, mc.memoria, convert(decimal(18,1),sum(bd.tamanho_mb/1024)) as tamanho_total_bd_GB, mc.data as data
...
EXEC @ReturnCode = msdb.dbo.sp_add_jobschedule @job_id=@jobId, @name=N'.', 
		...
		@active_start_time=70000, 
		@active_end_time=235959, 
		@schedule_uid=N'500e5259-ffb8-4eea-b777-b7d5df2e4d42'
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
EXEC @ReturnCode = msdb.dbo.sp_add_jobserver @job_id = @jobId, @server_name = N'(local)'
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
COMMIT TRANSACTION
...
/****** Object:  Job [E-mail: NFE Vivara, Amsterdam e Tiffany há mais de 10 minutos]    Script Date: 12/14/2012 11:24:50 ******/
BEGIN TRANSACTION
DECLARE @ReturnCode INT
SELECT @ReturnCode = 0
/****** Object:  JobCategory [[Uncategorized (Local)]]]    Script Date: 12/14/2012 11:24:50 ******/
...
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'passo1', 
		...
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [passo2]    Script Date: 12/14/2012 11:24:50 ******/
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'passo2', 
		...
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [Amsterdam e Tiffany]    Script Date: 12/14/2012 11:24:50 ******/
...
/****** Object:  Job [E-mail: Replicação Tiffany]    Script Date: 12/14/2012 11:24:50 ******/
BEGIN TRANSACTION
DECLARE @ReturnCode INT
SELECT @ReturnCode = 0
/****** Object:  JobCategory [[Uncategorized (Local)]]]    Script Date: 12/14/2012 11:24:50 ******/
...
exec monitordba.dbo.SP_MON_SEL_ReplicacaoParadaTiffany_CJ
go
exec monitordba.dbo.SP_MON_SEL_ReplicacaoParadaTiffany_DF
go
exec monitordba.dbo.SP_MON_SEL_ReplicacaoParadaTiffany_RJ
go', 
		@database_name=N'master', 
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
...
/****** Object:  Job [Email: Replicação Tiffany Conflitos]    Script Date: 12/14/2012 11:24:50 ******/
BEGIN TRANSACTION
DECLARE @ReturnCode INT
SELECT @ReturnCode = 0
/****** Object:  JobCategory [[Uncategorized (Local)]]]    Script Date: 12/14/2012 11:24:50 ******/
...
DECLARE @jobId BINARY(16)
EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N'Email: Replicação Tiffany Conflitos', 
		@enabled=1, 
		@notify_level_eventlog=0, 
		@notify_level_email=0, 
		@notify_level_netsend=0, 
...
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'.', 
		...
		@retry_interval=0, 
		@os_run_priority=0, @subsystem=N'TSQL', 
		@command=N'exec monitordba..PR_MONITOR_REP_CONFLITOS', 
		@database_name=N'master', 
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
EXEC @ReturnCode = msdb.dbo.sp_update_job @job_id = @jobId, @start_step_id = 1
...
EXEC @ReturnCode = msdb.dbo.sp_add_jobschedule @job_id=@jobId, @name=N'.', 
		...
		@freq_relative_interval=0, 
		@freq_recurrence_factor=0, 
		@active_start_date=20121018, 
		@active_end_date=99991231, 
		@active_start_time=73000, 
		@active_end_time=235959, 
		@schedule_uid=N'4bb76b47-08a4-4ff4-bdc3-3c55c4aff96f'
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
EXEC @ReturnCode = msdb.dbo.sp_add_jobserver @job_id = @jobId, @server_name = N'(local)'
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
COMMIT TRANSACTION
...
/****** Object:  Job [Email: Replicação Tiffany PRO]    Script Date: 12/14/2012 11:24:50 ******/
BEGIN TRANSACTION
DECLARE @ReturnCode INT
SELECT @ReturnCode = 0
/****** Object:  JobCategory [[Uncategorized (Local)]]]    Script Date: 12/14/2012 11:24:50 ******/
...
DECLARE @jobId BINARY(16)
EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N'Email: Replicação Tiffany PRO', 
		@enabled=1, 
		@notify_level_eventlog=0, 
		@notify_level_email=0, 
		@notify_level_netsend=0, 
...
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'.', 
		...
		@retry_interval=0, 
		@os_run_priority=0, @subsystem=N'TSQL', 
		@command=N'exec monitordba.dbo.SP_MON_SEL_ReplicacaoParadaTiffany_PRO
go

/*********************************************************************************/

exec monitordba.dbo.SP_MON_SEL_ReplicacaoParadaTiffany_PRO_CJ
go

exec monitordba.dbo.SP_MON_SEL_ReplicacaoParadaTiffany_PRO_DF
go

exec monitordba.dbo.SP_MON_SEL_ReplicacaoParadaTiffany_PRO_RJ
go', 
		@database_name=N'master', 
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
...
EXEC @ReturnCode = msdb.dbo.sp_add_jobschedule @job_id=@jobId, @name=N'.', 
		...
		@active_end_time=235959, 
		@schedule_uid=N'4c034b6d-8706-4ffa-816c-d2b50f291d3a'
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
EXEC @ReturnCode = msdb.dbo.sp_add_jobserver @job_id = @jobId, @server_name = N'(local)'
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
COMMIT TRANSACTION
...
/****** Object:  Job [Email: Replicação Tiffany PRO2]    Script Date: 12/14/2012 11:24:50 ******/
BEGIN TRANSACTION
DECLARE @ReturnCode INT
SELECT @ReturnCode = 0
/****** Object:  JobCategory [[Uncategorized (Local)]]]    Script Date: 12/14/2012 11:24:50 ******/
...
DECLARE @jobId BINARY(16)
EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N'Email: Replicação Tiffany PRO2', 
		@enabled=1, 
		@notify_level_eventlog=0, 
		@notify_level_email=0, 
		@notify_level_netsend=0, 
...
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'.', 
		...
		@os_run_priority=0, @subsystem=N'TSQL', 
		@command=N'/*
exec monitordba.dbo.SP_MON_SEL_ReplicacaoParadaTiffany_PRO2
go

------------------------------------------------------------------------------------------------------------

exec monitordba.dbo.SP_MON_SEL_ReplicacaoParadaTiffany_PRO2_CJ
go

exec monitordba.dbo.SP_MON_SEL_ReplicacaoParadaTiffany_PRO2_DF
go

exec monitordba.dbo.SP_MON_SEL_ReplicacaoParadaTiffany_PRO2_RJ
go
*/

EXEC MONITORDBA.DBO.pr_monitor_rep', 
		@database_name=N'master', 
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
EXEC @ReturnCode = msdb.dbo.sp_update_job @job_id = @jobId, @start_step_id = 1
...
EXEC @ReturnCode = msdb.dbo.sp_add_jobschedule @job_id=@jobId, @name=N'.', 
		...
		@freq_relative_interval=0, 
		@freq_recurrence_factor=0, 
		@active_start_date=20121124, 
		@active_end_date=99991231, 
		@active_start_time=0, 
		@active_end_time=235959, 
		@schedule_uid=N'35ca66d2-cea1-4980-8b46-8834b56743d5'
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
EXEC @ReturnCode = msdb.dbo.sp_add_jobserver @job_id = @jobId, @server_name = N'(local)'
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
COMMIT TRANSACTION
...
/****** Object:  Job [E-mail: Sinal de vida do cliente]    Script Date: 12/14/2012 11:24:50 ******/
BEGIN TRANSACTION
DECLARE @ReturnCode INT
SELECT @ReturnCode = 0
/****** Object:  JobCategory [[Uncategorized (Local)]]]    Script Date: 12/14/2012 11:24:50 ******/
...
EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N'E-mail: Sinal de vida do cliente', 
		...
		@owner_login_name=N'capta', @job_id = @jobId OUTPUT
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [passo1]    Script Date: 12/14/2012 11:24:51 ******/
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'passo1', 
		@step_id=1, 
		@cmdexec_success_code=0, 
		@on_success_action=1, 
...
/****** Object:  Job [E-mail: SQL no ar desde]    Script Date: 12/14/2012 11:24:51 ******/
BEGIN TRANSACTION
DECLARE @ReturnCode INT
SELECT @ReturnCode = 0
/****** Object:  JobCategory [[Uncategorized (Local)]]]    Script Date: 12/14/2012 11:24:51 ******/
IF NOT EXISTS (SELECT name FROM msdb.dbo.syscategories WHERE name=N'[Uncategorized (Local)]' AND category_class=1)
BEGIN
EXEC @ReturnCode = msdb.dbo.sp_add_category @class=N'JOB', @type=N'LOCAL', @name=N'[Uncategorized (Local)]'
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
...
/****** Object:  Job [Email: Taxa de Crescimento de Banco de Dados]    Script Date: 12/14/2012 11:24:51 ******/
BEGIN TRANSACTION
DECLARE @ReturnCode INT
SELECT @ReturnCode = 0
/****** Object:  JobCategory [[Uncategorized (Local)]]]    Script Date: 12/14/2012 11:24:51 ******/
...
EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N'Email: Taxa de Crescimento de Banco de Dados', 
		...
		@owner_login_name=N'capta', @job_id = @jobId OUTPUT
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [.]    Script Date: 12/14/2012 11:24:51 ******/
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'.', 
		@step_id=1, 
		@cmdexec_success_code=0, 
		@on_success_action=1, 
...
and c.flgAtivo <> 0
and c.ncodigo not in (75, 183) --vivara
...
/****** Object:  Job [E-mail: Versão Express próximo do limite]    Script Date: 12/14/2012 11:24:51 ******/
BEGIN TRANSACTION
DECLARE @ReturnCode INT
SELECT @ReturnCode = 0
/****** Object:  JobCategory [[Uncategorized (Local)]]]    Script Date: 12/14/2012 11:24:51 ******/
...
/****** Object:  Job [Envia E-mail]    Script Date: 12/14/2012 11:24:51 ******/
BEGIN TRANSACTION
DECLARE @ReturnCode INT
SELECT @ReturnCode = 0
/****** Object:  JobCategory [[Uncategorized (Local)]]]    Script Date: 12/14/2012 11:24:51 ******/
...
/****** Object:  Job [Espaço Total e Livre Tiffany]    Script Date: 12/14/2012 11:24:51 ******/
BEGIN TRANSACTION
DECLARE @ReturnCode INT
SELECT @ReturnCode = 0
/****** Object:  JobCategory [[Uncategorized (Local)]]]    Script Date: 12/14/2012 11:24:51 ******/
...
/****** Object:  Job [KILL USER]    Script Date: 12/14/2012 11:24:51 ******/
BEGIN TRANSACTION
DECLARE @ReturnCode INT
SELECT @ReturnCode = 0
/****** Object:  JobCategory [[Uncategorized (Local)]]]    Script Date: 12/14/2012 11:24:51 ******/
...
DECLARE @jobId BINARY(16)
EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N'KILL USER', 
		@enabled=1, 
		@notify_level_eventlog=0, 
		@notify_level_email=0, 
		@notify_level_netsend=0, 
...
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'.', 
		...
		@retry_interval=0, 
		@os_run_priority=0, @subsystem=N'TSQL', 
		@command=N'exec sp_killuser [captarmtdb]
...
EXEC @ReturnCode = msdb.dbo.sp_add_jobschedule @job_id=@jobId, @name=N'.', 
		...
		@active_end_date=99991231, 
		@active_start_time=225500, 
		@active_end_time=235959, 
		@schedule_uid=N'f1773b03-dbe6-4816-94d2-41c28aa10b31'
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
EXEC @ReturnCode = msdb.dbo.sp_add_jobserver @job_id = @jobId, @server_name = N'(local)'
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
COMMIT TRANSACTION
...
/****** Object:  Job [Limpeza de Log das Bases]    Script Date: 12/14/2012 11:24:51 ******/
BEGIN TRANSACTION
DECLARE @ReturnCode INT
SELECT @ReturnCode = 0
/****** Object:  JobCategory [[Uncategorized (Local)]]]    Script Date: 12/14/2012 11:24:51 ******/
...
/****** Object:  Job [Sinal de vida]    Script Date: 12/14/2012 11:24:51 ******/
BEGIN TRANSACTION
DECLARE @ReturnCode INT
SELECT @ReturnCode = 0
/****** Object:  JobCategory [[Uncategorized (Local)]]]    Script Date: 12/14/2012 11:24:51 ******/
...
EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N'Sinal de vida', 
		...
		@owner_login_name=N'capta', @job_id = @jobId OUTPUT
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [passo1]    Script Date: 12/14/2012 11:24:52 ******/
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'passo1', 
		@step_id=1, 
		@cmdexec_success_code=0, 
		@on_success_action=1, 
...
/****** Object:  Job [Tabelas Excedentes]    Script Date: 12/14/2012 11:24:52 ******/
BEGIN TRANSACTION
DECLARE @ReturnCode INT
SELECT @ReturnCode = 0
/****** Object:  JobCategory [[Uncategorized (Local)]]]    Script Date: 12/14/2012 11:24:52 ******/
IF NOT EXISTS (SELECT name FROM msdb.dbo.syscategories WHERE name=N'[Uncategorized (Local)]' AND category_class=1)
BEGIN
EXEC @ReturnCode = msdb.dbo.sp_add_category @class=N'JOB', @type=N'LOCAL', @name=N'[Uncategorized (Local)]'
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
...
EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N'Tabelas Excedentes', 
		...
		@owner_login_name=N'capta', @job_id = @jobId OUTPUT
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [Tabelas Excedentes]    Script Date: 12/14/2012 11:24:52 ******/
...
/****** Object:  Job [Versao Capta - desabilitado]    Script Date: 12/14/2012 11:24:52 ******/
BEGIN TRANSACTION
DECLARE @ReturnCode INT
SELECT @ReturnCode = 0
/****** Object:  JobCategory [[Uncategorized (Local)]]]    Script Date: 12/14/2012 11:24:52 ******/
