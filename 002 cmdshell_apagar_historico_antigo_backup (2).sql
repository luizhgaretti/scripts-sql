...
begin
	set @dt_historico = GETDATE() - @dias_filtro
	while @dt_historico <= GETDATE()- @dias_historico
	begin
		set @bd = ISNULL( ( select cComando from #tb_bd_bancoshist where nOrdem = @nOrdem ),'')
		set @sql2 = 'EXEC master.dbo.xp_cmdshell ' + char(39) + 'del e:\databases\backup_historico\' + @bd + '\' + @bd + '_backup_' + convert(varchar,datepart(yyyy,@dt_historico)) + '_' + right('00'+convert(varchar,datepart(mm,@dt_historico)),2)+ '_' + right('00'+convert(varchar,datepart(dd,@dt_historico)),2) + '_*.bak' + char(39) + ', NO_OUTPUT'
		exec(@sql2)
