USE [msdb]
GO
/****** Object:  StoredProcedure [dbo].[SP_DBA_EmailSQLServerHealth]    Script Date: 06/01/2012 09:48:51 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[SP_DBA_EmailSQLServerHealth]
(
...
INSERT INTO #diskspace(drive, diskspace) EXEC xp_fixeddrives     
--INSERT INTO #jobs_status EXEC msdb.dbo.sp_help_job  
INSERT INTO #jobs_status 
SELECT 
distinct
a.job_id,
HOST_NAME () AS originating_server,
a.name, 
enabled,
description,
start_step_id,    
B.name as category,
ISNULL(C.NAME,'') AS [owner],
...
d.last_run_date,    
--d.run_time,    
d.last_run_outcome as last_run_outcome
/*next_run_date,    
next_run_time,    
...
LEFT JOIN MASTER.SYS.syslogins C ON C.sid = A.owner_sid
LEFT JOIN MSDB..sysjobservers D ON D.job_id = A.job_id

--exec sp_serveroption [nbcaptasup11],'data access','true'
/*select *
into #jobs_status
from openquery([nbcaptasup11], 'set fmtonly off; exec msdb.dbo.sp_help_job')
*/
...
SELECT @strSubject = 'Relatório Diário Servidor ('+ CONVERT(VARCHAR(50), SERVERPROPERTY('servername')) + ')'    
...
SELECT 
	...
	<tr>    
	<th align="left" colspan="100%" bgColor="#000080">    
	<font face="Verdana" size="1" color="#FFFFFF">Databases Backup</font></th>    
 ...
 <th align="left" width="165" bgColor="#000080">    
	...
	<th align="left" width="136" bgColor="#000080">    
	<font face="Verdana" size="1" color="#FFFFFF">Size(MB)</font></th>    
	</tr> 
	'
...
SELECT TOP 20
	...
	'<td><font face="Verdana" size="1">' + ISNULL(CONVERT(VARCHAR(100), bkp.physical_device_name), '') +'</font></td>' +    
	...
	CASE
       WHEN  MST.backup_size = MST.compressed_backup_size THEN '<td><font face="Verdana" size="1">' +  ISNULL(CONVERT(VARCHAR(10), CAST((MST.backup_size/1024)/1024 AS DECIMAL(10,2))), '') +'</font></td>'  
       WHEN  MST.backup_size <> MST.compressed_backup_size THEN  '<td><font face="Verdana" size="1">' + ISNULL(CONVERT(VARCHAR(10), CAST((MST.compressed_backup_size/1024)/1024 AS DECIMAL(10,2))), '') +'</font></td>' 
       END  +  
	--'<td><font face="Verdana" size="1">' + ISNULL(CONVERT(VARCHAR(10), CAST((MST.backup_size/1024)/1024/1024 AS DECIMAL(10,2))), '') +'</font></td>' +  
	 '</tr>'     
FROM 
	backupset MST 
	join backupmediafamily bkp on bkp.media_set_id = mst.media_set_id and bkp.device_type = 2
WHERE
MST.backup_start_date BETWEEN 
	getdate() - 1
	and 
	getdate() + 1
	/*
	CAST(CONVERT(VARCHAR(4), DATEPART(yyyy, GETDATE())) + '-' + CONVERT(VARCHAR(2), DATEPART(mm, GETDATE())) + '-01' AS DATETIME) 
	AND 
	CAST(CONVERT(VARCHAR(5),DATEPART(yyyy, GETDATE())+1) + '-' + CONVERT(VARCHAR(2),DATEPART(mm, GETDATE())+1) + '-' + CONVERT(VARCHAR(2), DATEPART(dd, GETDATE())+1) AS DATETIME)  
	*/
ORDER BY 
...
SELECT 
	...
	<p><font face="Verdana" size="2"><b>Responsável:</b> '+@owner+'</font></p>  
