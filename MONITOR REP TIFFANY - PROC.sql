CREATE procedure [dbo].[SP_MON_SEL_ReplicacaoParadaTiffany]

as

declare @ultima_replicacao datetime

set @ultima_replicacao = ( select MAX(i.dConclusao) from Tb_MON_Mov_IndicadoresMonitor i where i.nCliente = 73 and i.nTipoMonitor = 2 )
...
begin
 ...
 declare @TableHTML varchar(max)
	...
	<font face="Verdana" size="2" color="#FFFFFF">Dt. Conclusão</font></b></td>
			...
			<tr>  
				'
					if exists (select name from tempdb..sysobjects where name like '#tb_dba_replicacaotiffany%')
						drop table #tb_dba_replicacaotiffany
	...
	@subject = 'Replicação Parada/Em execução (Tiffany)',    
	@body = @TableHTML,    
	@body_format = 'HTML' ;	
	end	
