...
alter PROCEDURE usp_backup_database  @typebackup tinyint , @directorypath varchar(2000) with encryption
AS
...
BEGIN
 ...
 BEGIN
  ...
  begin
   ...
   begin 
    set @backupsql = ''
    set @backupsql = @backupsql + 'backup log ' + @databasename + ' to  disk = N''' + @fullpath + 'BKLOG_' + @apagar  + '_'+ replace(CONVERT(VARCHAR(5),GETDATE(),114),':','')+'_' + @databasename + '.trn'' with init, compression '
    exec (@backupsql)
   end
  end
