...
alter PROCEDURE usp_backup_database  @typebackup tinyint , @directorypath varchar(2000) 
AS
...
BEGIN
 ...
 BEGIN
  ...
  begin
   set @backupsql = ''
   set @backupsql = @backupsql + 'backup database ' + @databasename + ' to  disk = N''' + @fullpath + 'BKFULL_' + @databasename + '.bak'' with retaindays = 3,compression'
   exec (@backupsql)
  end
