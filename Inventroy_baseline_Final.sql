/*
Script	: Inventory_baseline.sql
Purpose	: Collecting SQL Server 2000/2005/2008 baseline inventory: 
Requirements:   SQL Server 2008 Express Edition 
Version: 1.0
*/
...
-- Step 3: Reading registry keys for IP,Binaries,Startup type ,startup username, errorlogs location and domain.
		...
		EXEC master.dbo.xp_regread    @rootkey = 'HKEY_LOCAL_MACHINE'    ,@key = @RegKey    ,@value_name = 'TcpPort'    ,@value = @TcpPort OUTPUT
...
-- Step 5: printing Server details 
			...
			case serverproperty('productversion') when '12.0.2000.8' then 'SQL Server 2014 RTM'
								when '11.0.2100.60' then 'SQL Server 2012 RTM'
								when '11.0.3000.0' then 'SQL Server 2012 SP1'
								when '11.1.3000.0' then 'SQL Server 2012 SP1'
								when '11.0.5058.0' then 'SQL Server 2012 SP2'
								when '11.2.5058.0' then 'SQL Server 2012 SP2'
								when '10.50.1600.1' then 'SQL Server 2008 R2 RTM'
								when '10.50.2500.0' then 'SQL Server 2008 R2 SP1'
								when '10.51.2500.0' then 'SQL Server 2008 R2 SP1'
								when '10.50.4000.0' then 'SQL Server 2008 R2 SP2'
								when '10.52.4000.0' then 'SQL Server 2008 R2 SP2'
								when '10.50.6000.34' then 'SQL Server 2008 R2 SP3'
								when '10.53.6000.34' then 'SQL Server 2008 R2 SP3'
								when '10.0.1600.22' then 'SQL Server 2008 RTM'
								when '10.0.2531.0' then 'SQL Server 2008 SP1'
								when '10.1.2531.0' then 'SQL Server 2008 SP1'
								when '10.0.4000.0' then 'SQL Server 2008 SP2'
								when '10.2.4000.0' then 'SQL Server 2008 SP2'
								when '10.0.5500.0' then 'SQL Server 2008 SP3'
								when '10.3.5500.0' then 'SQL Server 2008 SP3'
								when '10.0.6000.29' then 'SQL Server 2008 SP4'
								when '10.4.6000.29' then 'SQL Server 2008 SP4'
								when '9.00.1399.06' then 'SQL Server 2005 RTM'
								when '9.0.2047' then 'SQL Server 2005 SP1'
								when '9.0.3042' then 'SQL Server 2005 SP2'
								when '9.0.4035' then 'SQL Server 2005 SP3'
								when '9.0.5000' then 'SQL Server 2005 SP4'
								when '8.0.194' then 'SQL Server 2000 RTM'							
								when '8.0.384' then 'SQL Server 2000 SP1'
								when '8.0.532' then 'SQL Server 2000 SP2'
								when '8.0.760' then 'SQL Server 2000 SP3'
								when '8.0.2039' then 'SQL Server 2000 SP4'
								when '7.0.623' then 'SQL Server 7.0'
								when '7.0.699' then 'SQL Server 7.0'
								when '7.0.842' then 'SQL Server 7.0'
								when '7.0.961' then 'SQL Server 7.0'
								when '7.0.1063' then 'SQL Server 7.0' END Version_sql,                      
